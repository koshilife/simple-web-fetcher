...
module SimpleWebFetcher
  ...
  class CommandOptionParserTest < BaseTest
    def setup
      ...
      begin
        option = SimpleWebFetcher::CommandOptionParser.new([@valid_url1], StringIO.new)
        @help_msg = option.instance_variable_get('@parser').help
      rescue StandardError => e
        puts e
        @help_msg = 'Error: fail to get help messages.'
      end
    end

    def test_that_it_parse_single_url
      argv = [@valid_url1]
      parser = @subject.new(argv, @dummy_io)
    ...
    def test_that_it_exit_if_invalid_option
      ...
      assert_equal(1, e.status)
      assert_equal("OptionParseError: OptionParser::InvalidOption is occured. invalid option: -x\n#{@help_msg}\n",
                   @dummy_io.string)
    end
    ...
    def test_that_it_exit_if_exe_mode_is_unknown
      ...
      assert_equal(1, e.status)
      assert_equal("Error: ExeMode is unknown. unknown_exe_mode\n#{@help_msg}\n",
                   @dummy_io.string)
    end
  end
end
